...
| **自动（无需触发）** | **被动 RAG：提问时自动检索知识库，注入相关笔记** |

---

## 功能〇：被动 RAG（自动知识检索）

> 不用 `@brain`，正常聊天也能自动调取你的知识库。

### 怎么工作

每次你提问时，Agent 自动：

1. 提取问题中的实体词（项目名、技术术语、人名）
2. 在 `D:\知识库` 中 `grep` 搜索这些词
3. 命中笔记 → 提取相关段落 → 注入回答
4. 回答中自动引用 `[[笔记名]]`
...
Agent：（后台自动搜索到 [[KSTAR熔断机制]]）
      → 你的知识库里记录的是 80% 阈值 + 每知识点最多 2 轮。
        参考 [[KSTAR熔断机制]]，要不要根据实际数据调一下？
```
...
Agent：（命中了 [[小洛记忆系统架构]]）
      → 你之前设计的是短期 20 轮窗口 + 长期 JSON 持久化双层结构。
        参考 [[小洛记忆系统架构]] 和 [[Companion Agent 核心设计]]。
```
